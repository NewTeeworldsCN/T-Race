...
void CPlayer::OnPredictedInput(CNetObj_PlayerInput *pNewInput)
{
	...
	if(m_pCharacter && !m_Paused && !(pNewInput->m_PlayerFlags & PLAYERFLAG_SPEC_CAM) && !m_Sleep)
...
void CPlayer::OnDirectInput(CNetObj_PlayerInput *pNewInput)
{
	...
	if(((pNewInput->m_PlayerFlags & PLAYERFLAG_SPEC_CAM) || GetClientVersion() < VERSION_DDNET_PLAYERFLAG_SPEC_CAM) && ((!m_pCharacter && m_Team == TEAM_SPECTATORS) || m_Paused || m_DeadSpec) && m_SpectatorId == SPEC_FREEVIEW)
...
void CPlayer::OnPredictedEarlyInput(CNetObj_PlayerInput *pNewInput)
{
	...
	if(m_pCharacter && !m_Paused && !(m_PlayerFlags & PLAYERFLAG_SPEC_CAM) && !m_Sleep)
